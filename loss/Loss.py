...
class LossWithGAN_STE(nn.Module):
    def __init__(self, logPath, extractor, Lamda, lr, betasInit=(0.5, 0.9), hasmask=True):
        super(LossWithGAN_STE, self).__init__()
        self.l1 = nn.L1Loss()
        self.extractor = extractor
        self.discriminator = Discriminator_STE(
            3, hasmask)  # local_global sn patch gan
        self.D_optimizer = torch.optim.Adam(
            self.discriminator.parameters(), lr=lr, betas=betasInit)
...
class LossWithGAN_STE_WithoutMask(LossWithGAN_STE):
    def forward(self, input, x_o1, x_o2, x_o3, output, gt, count, epoch):
        ...
        # output_comp = mask * input + (1 - mask) * output
       # import pdb;pdb.set_trace()
        holeLoss = 10 * self.l1(output,  gt)
        # validAreaLoss = 2*self.l1( output, gt)
        ...
        imgs1 = F.interpolate(gt, scale_factor=0.25)
        imgs2 = F.interpolate(gt, scale_factor=0.5)
        msrloss = 8 * self.l1(x_o3, gt) + \
            6 * self.l1(x_o2, imgs2) +\
            5 * self.l1(x_o1, imgs1)
